...
// Función que devuelve una subcadena UTF-8 segura
std::string utf8_substr(const std::string &str, size_t byte_offset, size_t byte_length)
{
    size_t str_len = str.size();

    // Asegurarse de que el offset no esté fuera de los límites
    if (byte_offset > str_len)
    {
        throw std::out_of_range("Offset fuera de los límites de la cadena.");
    }

    // Iterar sobre la cadena para encontrar el inicio de un carácter UTF-8 válido
    size_t start = byte_offset;
    while (start < str_len && (str[start] & 0xC0) == 0x80)
    {
        // Si estamos en medio de un carácter UTF-8 (byte de continuación), retroceder
        start--;
    }

    // Del mismo modo, iterar sobre la longitud para asegurarnos de que no cortamos al final de un carácter UTF-8
    size_t end = start + byte_length;
    if (end > str_len)
    {
        end = str_len;
    }
    while (end < str_len && (str[end] & 0xC0) == 0x80)
    {
        // Si estamos en medio de un carácter UTF-8, ajustar el final
        end--;
    }

    // Devolver la subcadena ajustada
...
{
    llm_tokenizer_bpe(const llama_vocab &vocab, const std::string &lang) : vocab(vocab)
    {
        ...
        case LLAMA_VOCAB_PRE_TYPE_LLAMA3:
            ...
            regex_exprs = {
                ...
                "|\\s*[\\r\\n]+"
                "|\\s+(?!\\S)"
                "|[\\p{InArabic}\\p{InHebrew}\\p{InCyrillic}"
                "\\p{InHiragana}\\p{InKatakana}\\p{InCJKUnifiedIdeographs}"
                "\\p{InLatinExtendedA}\\p{InLatinExtendedB}\\p{InLatinExtendedAdditional}"
                "]+",
            };
            break;
...
std::u32string utf8_to_utf32(const std::string &utf8_str)
{
    ...
    while (i < utf8_str.size())
    {
        ...
        else if ((byte & 0xE0) == 0xC0)
        {
            ...
            utf32_char = ((byte & 0x1F) << 6) | (utf8_str[i + 1] & 0x3F);
            i += 2;
        }
        else if ((byte & 0xF0) == 0xE0)
        {
            if (i + 2 >= utf8_str.size())
                throw std::runtime_error("Invalid UTF-8 sequence");
            utf32_char = ((byte & 0x0F) << 12) | ((utf8_str[i + 1] & 0x3F) << 6) | (utf8_str[i + 2] & 0x3F);
            i += 3;
        }
        else if ((byte & 0xF8) == 0xF0)
        {
            if (i + 3 >= utf8_str.size())
                throw std::runtime_error("Invalid UTF-8 sequence");
            utf32_char = ((byte & 0x07) << 18) | ((utf8_str[i + 1] & 0x3F) << 12) | ((utf8_str[i + 2] & 0x3F) << 6) | (utf8_str[i + 3] & 0x3F);
            i += 4;
        }
        else
        {
        ...
        utf32_result.push_back(utf32_char);
    }

    return utf32_result;
}

std::string detect_language(const std::string &text, bool in_vocab)
{
    ...
    for (char32_t c : utf32_text)
    {
        ...
        else if (c >= 0x0590 && c <= 0x05FF && (utf32_text.find(U"יי") != std::u32string::npos))
        {
            return "YIDDISH";
        }
        ...
        else if ((c >= 0x3040 && c <= 0x309F) || (c >= 0x30A0 && c <= 0x30FF) || (c >= 0x4E00 && c <= 0x9FFF))
        {
            return "JAPANESE";
        }
        else if ((c >= 0x0100 && c <= 0x017F) || (utf32_text.find(U"çöüğşıÇÖÜĞŞI") != std::u32string::npos))
        {
            return "TURKISH";
        }
        else if ((c >= 0x0100 && c <= 0x017F) || (utf32_text.find(U"áéíóöőúüűÁÉÍÓÖŐÚÜŰ") != std::u32string::npos))
        {
            return "HUNGARIAN";
        }
        else if (utf32_text.find(U"çãõáéíóúÇÃÕÁÉÍÓÚ") != std::u32string::npos)
        {
            return "PORTUGUESE";
        }
        else if (utf32_text.find(U"ñáéíóúüÑÁÉÍÓÚÜ") != std::u32string::npos)
        {
            return "SPANISH";
        }
    }
...
std::vector<llama_vocab::id> llama_tokenize_internal(const llama_vocab &vocab, std::string raw_text, bool add_special, bool parse_special, bool in_vocab)
{
    ...
    case LLAMA_VOCAB_TYPE_BPE:
    {
        std::string lang = detect_language(raw_text, in_vocab);
        llm_tokenizer_bpe tokenizer(vocab, lang);
        if (add_special)
        {
        ...
        for (const auto &fragment : fragment_buffer)
        {
            ...
            {
                auto raw_text = fragment.raw_text.substr(fragment.offset, fragment.length);
                // auto raw_text = utf8_substr(fragment.raw_text, fragment.offset, fragment.length);
...
int32_t llama_tokenize_impl(
    ...
    bool parse_special)
{
    auto res = llama_tokenize_internal(vocab, std::string(text, text_len), add_special, parse_special, false);
